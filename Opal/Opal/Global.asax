...
<script runat="server">
    ...
    {
        try {
            Opal.WebSite.GetInstance();
            SiteMap.Provider.SiteMapResolve += new SiteMapResolveEventHandler(Opal.Providers.CustomXmlSitemapProvider.Resolve);
        } catch (Exception ex)
        {
            throw ex;
        }
    ...
    void Application_AuthorizeRequest(object sender, EventArgs e)
    {
		...
		if (currentURL.EndsWith(".aspx"))
		{
			...
			if (!System.IO.File.Exists(Server.MapPath(currentURL)))
			{
				...
				if (pageId == null)
					pageId = Opal.WebSite.GetInstance().HomepageId;
